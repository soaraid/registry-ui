    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col justify-center gap-8 px-4 py-10 md:px-8 lg:flex-row lg:items-center lg:justify-between">
        <div className="max-w-xl space-y-6">
          <Badge className="w-fit bg-emerald-400/10 text-emerald-200">Session-gated registry access</Badge>
          <div className="space-y-4">
            <div className="inline-flex rounded-2xl border border-white/10 bg-white/5 p-4">
              <ShieldCheck className="h-7 w-7 text-emerald-300" />
            </div>
            <h1 className="text-4xl font-semibold tracking-tight md:text-5xl">
              Protect your registry control plane before it leaves localhost.
            </h1>
            <p className="text-base leading-7 text-muted-foreground">
              Single-user session auth is now env-driven, so you can ship this UI in Docker and keep the
              registry tooling behind one credential while you prepare a fuller auth model.
            </p>
          </div>
        </div>
